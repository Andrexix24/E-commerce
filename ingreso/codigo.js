...
function agregarAlCarrito(productoEncontrado){
    carrito.push(productoEncontrado); // Agregar producto al carrito
    texto = `${productoEncontrado.nombre} (Cantidad: ${productoEncontrado.cantidad})`;
    p.textContent = `Producto agregado al carrito: ${texto}`;
    console.log(carrito)
...
p1.textContent = (`Total del carrito: ${total}$`);
respuesta.appendChild(p1)
}
